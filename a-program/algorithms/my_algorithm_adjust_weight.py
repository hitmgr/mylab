...
class Server(fedbase.BasicServer):
    def initialize(self, *args, **kwargs):
        ...
        algo_params = {
            ...
            'selected_round': 5,  # 每隔多少轮更新一次委员会
            'adjust_rate': 0.05,  # 自适应权重调整速率
            'patience': 5,  # 耐心轮数，用于判断全局准确率的变化
        }
    ...
    def adjust_weights(self, current_global_acc):
        ...
        # 宏观调整：基于训练阶段
        if self.current_round < self.num_rounds / 3:
            # 初期阶段
            self.w1, self.w2 = 0.7, 0.3
        elif self.current_round < 2 * self.num_rounds / 3:
            # 中期阶段
            self.w1, self.w2 = 0.5, 0.5
        else:
            # 后期阶段
            self.w1, self.w2 = 0.3, 0.7
        
        # 微观调整：基于准确率的动态调整
        if current_global_acc > self.prev_global_acc:
            self.stable_rounds += 1
            if self.stable_rounds >= self.patience:
                self.w2 = min(1.0, self.w2 + self.adjust_rate / 10)
                self.w1 = 1.0 - self.w2
                self.stable_rounds = 0
        elif current_global_acc < self.prev_global_acc:
            self.stable_rounds += 1
            if self.stable_rounds >= self.patience:
                self.w1 = min(1.0, self.w1 + self.adjust_rate / 10)
                self.w2 = 1.0 - self.w1
                self.stable_rounds = 0
        else:
            self.stable_rounds = 0
